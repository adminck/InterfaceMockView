<template>
    <div class="router-history">
        <el-tabs
           :closable="!(historys.length==1&&this.$route.name=='person')"
           @contextmenu.prevent.native="openContextMenu($event)"
           @tab-click="changeTab"
           @tab-remove="removeTab"
           type="card"
           v-model="activeValue">
            <el-tab-pane :key="item.name" :label="item.meta.title" :name="item.name" v-for="item in historys"></el-tab-pane>
        </el-tabs>
...
<script>
    export default {
        ...
        created() {
            ...
            const initHistorys = [{name: 'person', meta: {title: '首页'}}]
            this.historys = JSON.parse(sessionStorage.getItem('historys')) || initHistorys
            this.setTab(this.$route)
        },
        methods: {
            openContextMenu(e) {
                if (this.historys.length == 1 && this.$route.name == 'person') {
                    return false
                }
                if (e.srcElement.id) {
